...
bowtie2-build -f Tcongo_genome/TriTrypDB-46_TcongolenseIL3000_2019_Genome.fasta.gz Tcongo_db --threads 32 -q && echo -e "\nBowtie2 database successfully built\n"
...
do
  ...
  bowtie2 -p 64 -x Tcongo_db -1 $i1 -2 $i2 --very-sensitive-local | samtools sort -o ${i1%%_1.fq.gz}_sorted.bam -
done
